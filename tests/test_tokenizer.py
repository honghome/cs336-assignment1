...
# `resource` is Unix-only; on Windows we fall back to a no-op memory_limit.
try:
...
def memory_limit(max_mem):
    def decorator(f):
        if resource is None:
            # No RLIMIT_AS on this platform (e.g., Windows). Skip enforcement.
            return f
...
@pytest.mark.skipif(
    not sys.platform.startswith("linux") and resource is None,
    reason="rlimit support for non-linux systems is spotty; memory_limit becomes a no-op on Windows so this test only exercises encode_iterable end-to-end (no actual memory cap).",
)
...
@pytest.mark.skipif(
    not sys.platform.startswith("linux") and resource is None,
    reason="rlimit support for non-linux systems is spotty; without the cap this test cannot exhibit its expected OOM failure.",
)
@pytest.mark.xfail(
    sys.platform.startswith("linux") or resource is not None,
